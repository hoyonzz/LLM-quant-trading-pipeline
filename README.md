...
---
# ⚠️ [이관 안내] 프로젝트 종료 및 신규 시스템으로의 이전

> **[Notice]**
> 본 프로젝트(**LQSP**)는 더 이상 유지보수되지 않으며, 차세대 시스템인 **IronMan-Quant-Bot (IQB)**으로 공식 통합되었습니다.
>
> **🛑 개발 중단 사유 (Legacy Limitations):**
> 1. **동기식 처리(Synchronous)의 한계:** 시세 수신과 주문 로직이 동시에 처리되지 않아 발생하는 딜레이(Latency) 문제.
> 2. **실시간 감시 부재:** Websocket 미적용으로 인해 급변하는 시장 상황에 즉각 대응 불가.
> 3. **상태 관리의 어려움:** 파일 기반 로그 시스템의 한계로 데이터 무결성 보장 실패.
>
> **🚀 신규 프로젝트 (New Architecture):**
> - **비동기(Asyncio) 아키텍처** 도입으로 실시간성 확보.
> - **SQLite DB** 도입으로 데이터 안정성 강화.
> - **Gemini AI** 도입으로 정성적 분석(뉴스) 기능 추가.
>
> 👉 **신규 프로젝트 바로가기:** [IronMan-Quant-Bot Repository 링크](https://github.com/hoyongzz/IronMan-Quant-Bot)
>
> *이 저장소는 V1 아키텍처 연구를 위한 아카이브(Archive) 용도로만 보존됩니다.*
